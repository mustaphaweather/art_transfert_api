...
def image_loader_path_content(image_path):
    response = requests.get(image_path)
    image = Image.open(BytesIO(response.content))
    # fake batch dimension required to fit network's input dimensions
    size_ = image.size
    image = loader(image).unsqueeze(0)
    return [image.to(device, torch.float),size_]

def image_loader_path_style(image_path,ctn_size):
    response = requests.get(image_path)
    image = Image.open(BytesIO(response.content))
...
content_img_req = image_loader_path_content("https://widoobizweb.s3.amazonaws.com/wp-content/uploads/2019/05/02124054/rsz-simon-migaj-421505-unsplash-1072x568.jpg")
content_img = content_img_req[0]
content_img_size = content_img_req[1]

style_img = image_loader_path_style("https://static.euronews.com/articles/297534/1280x720_297534.jpg" , content_img_size)
